...
def plot_token_distribution_multi(
    ...
    for ax, ds_name in zip(axes, dataset_names):
        ...
        for counts, raw_label in zip(all_counts, raw_labels):
            ...
            is_zipcal = label == "ZipCal"
            zorder = 5 if is_zipcal else 3
            lw = 4.0 if is_zipcal else 2.8
            (line,) = ax.plot(ranks, freqs_norm, linewidth=lw, alpha=0.7, label=label, color=color, zorder=zorder)
        ...
        (ref_line,) = ax.plot(
            zipf_ranks, zipf_vals, "k--", linewidth=1, alpha=0.55, label="Theoretical Zipf"
        )
        ...
        ax.set_title(DATASET_NAME_MAP.get(ds_name, ds_name))
        ax.grid(False)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--datasets",
        nargs="+",
        default=[ "gsm8k", "winogrande", "hellaswag"],
        required=False,

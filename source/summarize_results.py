...
def summarize_results(csv_path):
    ...
    summary = df.groupby(['model', 'task', 'metric', 'sparsity', 'nsamples', 'calibration_datasets', 'pruning_type'])['pruned_value'].mean().unstack()
    
    # Get original values
    orig_values = df.groupby(['model', 'task', 'metric', 'sparsity', 'nsamples', 'calibration_datasets'])['original_value'].first()
    ...
    with open(output_md, "w") as f:
        ...
        avg_summary = df.groupby(['model', 'sparsity', 'nsamples', 'calibration_datasets', 'pruning_type'])['pruned_value'].mean().unstack()
        avg_orig = df.groupby(['model', 'sparsity', 'nsamples', 'calibration_datasets'])['original_value'].mean()
        ...
        calib_avg_summary = df.groupby(['model', 'task', 'metric', 'sparsity', 'nsamples', 'pruning_type'])['pruned_value'].mean().unstack()
        calib_avg_orig = df.groupby(['model', 'task', 'metric', 'sparsity', 'nsamples'])['original_value'].mean()
        ...
        f.write("\n\n## Global Average (Across Tasks and Calibration Datasets)\n\n")
        global_summary = df.groupby(['model', 'sparsity', 'nsamples', 'pruning_type'])['pruned_value'].mean().unstack()
        global_orig = df.groupby(['model', 'sparsity', 'nsamples'])['original_value'].mean()

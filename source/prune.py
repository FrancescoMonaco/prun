...
def get_tokenized_data(dataset, tokenizer, max_length=128, return_tensors=False):
    ...
    for item in dataset:
        ...
        if return_tensors:
            ...
            processed_dataset.append({"input_ids": encoded["input_ids"].squeeze(0), "attention_mask": encoded["attention_mask"].squeeze(0)})
        else:
...
if __name__ == "__main__":
    ...
    calibration_data_dicts = use_embedding_for_sampling( # Renamed from _tuples to _dicts
        [tokenized_dataset],
        model,
    ...
    tokenized_eval_data = get_tokenized_data(eval_dataset, tokenizer, return_tensors=False)

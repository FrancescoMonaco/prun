...
def main():
    parser = argparse.ArgumentParser(
        description="Window-length ablation: sweep calibration sequence lengths"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Calibration selection strategy to use across all window lengths",
    )
    parser.add_argument(
        "--window_lengths",
        nargs="+",
        type=int,
        default=[16, 512, 1024, 2048],
        help="Sequence lengths (window sizes) to sweep over",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_csv",
        type=str,
        default="results/window_length_results.csv",
    ...
    # Load raw datasets once; re-tokenize per window length inside the loop
    ...
    for d_name in args.datasets:
        ...
        raw_datasets.append((d_name, raw_dataset))

    # Lazy-loaded sentence transformer (reused when not using least_perplexity)
    sentence_transformer = None

    # 3. Main loop: sweep over window lengths
    for win_len in args.window_lengths:
        log.info(f"\n--- Window length: {win_len} ---")

        # Define save path (includes window length for caching)
        ...
        if use_disk_cache and os.path.exists(os.path.join(save_path, "config.json")):
            ...
            # Re-tokenize calibration data with the current window length
            log.info(f"Tokenizing calibration data with max_length={win_len}...")
            all_tokenized_datasets = []
            for d_name, raw_ds in raw_datasets:
                tokenized = get_tokenized_data(raw_ds, tokenizer, d_name, max_length=win_len)
            ...
            if args.compression_type == "2ssp":
                ...
                num_chunks = all_ids.size(0) // win_len
                if num_chunks == 0:
                    log.warning(f"Only {all_ids.size(0)} tokens, need >= {win_len}. Using all tokens as one sample.")
                    calibration_2ssp = [all_ids.unsqueeze(0)]
                else:
                    calibration_2ssp = [all_ids[i * win_len : (i + 1) * win_len].unsqueeze(0) for i in range(num_chunks)]
        ...
        log.info(f"Evaluating compressed model (window_length={win_len})...")
        pruned_raw = evaluate_model(args.model, pruned_model, tokenizer, args.eval_tasks)
        pruned_metrics = process_results(pruned_raw)

        # 5. Save results to CSV
        log.info(f"Saving results for window_length={win_len} to {args.output_csv}...")
        rows = []
        ...
        # Reload original model before next window length (compression modifies in-place)
        if win_len != args.window_lengths[-1]:
            log.info("Reloading original model for next window length...")
    ...
    log.info("All window-length experiments finished successfully.")
